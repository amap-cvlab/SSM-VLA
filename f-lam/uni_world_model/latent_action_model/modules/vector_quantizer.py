...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(self, num_latents, latent_dim, beta, remap=None, unknown_index="random",
                 sane_index_shape=False, legacy=True):
        super().__init__()
        self.n_e = num_latents
    ...
    def codebook(self, min_encoding_indices, shape):
        bz = shape[0]
        z_q = self.embedding(min_encoding_indices).view(shape)
        # reshape back to match original input shape
        z_q = z_q.reshape(bz, -1, z_q.shape[-1])
