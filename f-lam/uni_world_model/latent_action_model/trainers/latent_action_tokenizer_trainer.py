...
from uni_world_model.latent_action_model.trainers.trainer_utils import visualize_latent_motion_reconstruction, visualize_latent_motion_depth_reconstruction
...
def depth_image_fn(depth_images, normalize=True, max_depth=10.0, to_tensor=True):
    # If input is a tensor, use it directly; else convert from numpy/list
    B = depth_images.shape[0]
    if isinstance(depth_images, torch.Tensor):
        depth_tensor = rearrange(depth_images, 'b t c h w -> (b t) c h w')
        if depth_tensor.dim() == 3:
            depth_tensor = depth_tensor.unsqueeze(1)  # (N, 1, H, W)
    ...
    depth_tensor_resized = rearrange(depth_tensor_resized, '(b t) c h w -> b t c h w', b=B) 

    if to_tensor:
        return depth_tensor_resized
    else:
        return depth_tensor_resized.numpy()

def depth_image_back_fn(depth_images, normalize=True, max_depth=10.0):
    B = depth_images.shape[0]
    # If input is a tensor, use it directly; else convert from numpy/list
    if isinstance(depth_images, torch.Tensor):
        depth_tensor = rearrange(depth_images, 'b t c h w -> (b t) c h w')
        ...
        depth_tensor = torch.from_numpy(np_depth).unsqueeze(1)  # (N, 1, H, W)

    resize = T.Resize((200, 200), interpolation=T.InterpolationMode.NEAREST)
    depth_tensor_resized = torch.stack([resize(d) for d in depth_tensor])
    depth_tensor_resized = rearrange(depth_tensor_resized, '(b t) c h w -> b t c h w', b=B)
...
class LatentActionTokenizerTrainer(nn.Module):
    ...
    def eval_latent_motion_reconstruction(self, visualization_dir):
        ...
        orig_rgb_seq = torch.cat([batch['rgb_initial'], batch['rgb_future']], dim=1) # (b, t+1, c, h, w)
        rgb_seq = self.rgb_preprocessor(orig_rgb_seq, train=True)
        depth_seq = torch.cat([batch['depth_initial'], batch['depth_future']], dim=1) # (b, t+1, 1, h, w)
        depth_seq = depth_image_fn(depth_seq).to(self.device)
        rgb_seq = torch.cat([rgb_seq, depth_seq], dim=2)  #

        self.latent_action_tokenizer.eval()
        outputs = self.latent_action_tokenizer(
            cond_pixel_values=rgb_seq[:,:1],
            target_pixel_values=rgb_seq[:,1:],
            return_recons_only=True
        )
        rgb_seq = rgb_seq[:,:,:3]
        recons_rgb_future = self.rgb_preprocessor.post_process(outputs["recon_pixel_values"]).detach().cpu()  # (b, t, c, h, w)
        recons_depth_future = depth_image_back_fn(outputs["recon_depth_values"]).detach().cpu()  # (b, t, 1, h, w)
        gt_latent_motion_ids = outputs["indices"].detach().cpu() # (b, t, per_latent_motion_len)
        # orig_rgb_seq = orig_rgb_seq.detach().cpu()
        orig_rgb_seq = self.rgb_preprocessor.post_process(rgb_seq).detach().cpu()
        orig_depth_seq = depth_image_back_fn(depth_seq).detach().cpu()

        for i in range(orig_rgb_seq.shape[0]):
            visualize_latent_motion_reconstruction(
                initial_frame=orig_rgb_seq[i][:1],
        ...
        for i in range(orig_depth_seq.shape[0]):
            visualize_latent_motion_depth_reconstruction(
                initial_frame=orig_depth_seq[i][:1],
                next_frame=orig_depth_seq[i][1:],
                recons_next_frame=recons_depth_future[i],
                latent_motion_ids=gt_latent_motion_ids[i],
                path=os.path.join(visualization_dir, f"{self.process_index}-{i}-depth.png")
            )
    ...
    def calculate_loss(self, batch, train):
        ...
        depth_seq = depth_image_fn(depth_seq, to_tensor=True).to(self.device)
        rgb_seq = torch.cat([rgb_seq, depth_seq], dim=2)  # (b, t, c+1, h, w)
        # compute loss
        outputs = self.latent_action_tokenizer(
            cond_pixel_values=rgb_seq[:,:1],
            target_pixel_values=rgb_seq[:,1:]
