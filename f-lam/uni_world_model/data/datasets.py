...
class NpzDataset_for_MotoGPT_Video(Dataset):
    def __init__(
        ...
        self.dummy_rgb_initial = torch.zeros(1, 3, rgb_shape[0], rgb_shape[1], dtype=torch.uint8)
        self.dummy_rgb_future = torch.zeros(sequence_length, 3, rgb_shape[0], rgb_shape[1], dtype=torch.uint8)
        self.dummy_depth_initial = torch.zeros(1, 1, rgb_shape[0], rgb_shape[1], dtype=torch.float32)
        self.dummy_depth_future = torch.zeros(sequence_length, 1, rgb_shape[0], rgb_shape[1], dtype=torch.float32)
        self.dummy_latent_mask = torch.zeros(sequence_length)
    ...
    def extract_frames(self, npz_basename, delta_t, 
        ...
        def _extract_frame(npz_idx):
            ...
            try:
                ...
                frame_depth = np.load(npz_path)['depth_static']
            except Exception as e:
                raise e
            ...
            frame_depth = torch.from_numpy(rearrange(frame_depth, 'h w c -> c h w'))
            if self.rgb_preprocessor is not None:
                frame  = self.rgb_preprocessor(frame)
        ...
        rgb_initial[0], depth_initial[0] = _extract_frame(start_npz_idx)

        for i in range(self.sequence_length):
            next_npz_idx = start_npz_idx+(i+1)*delta_t
            try:
                rgb_future[i], depth_future[i] = _extract_frame(next_npz_idx)
                latent_mask[i] = 1
            except:
                break
...
class NpzDataset_for_Libero_Video(Dataset):
    def __init__(
        self, split, skip_frame, 
        sequence_length,
        npz_dir=None, rgb_shape=(128, 128), 
        rgb_preprocessor=None, max_skip_frame=None, 
        ...
        *args, **kwargs
        ...
        # 路径设置
        if split == 'train':
            self.dataset_path = os.path.join(npz_dir, 'libero_90_converted')
        elif split == 'val':
            self.dataset_path = os.path.join(npz_dir, 'libero_10_converted')
            
        # 加载元数据
        if npz_metadata_path is None:
            if split == 'train':
                npz_metadata_path = os.path.join("libero_90_converted.json")
            elif split == 'val':
                npz_metadata_path = os.path.join("libero_10_converted.json")
        
        with open(npz_metadata_path, 'r') as f:
            self.episode_info_list = json.load(f)
        
        # 构建episode查找表
        self.episode_list = [f[0] for f in self.episode_info_list]
        self.num_step_per_episode = [f[1] - sequence_length*skip_frame for f in self.episode_info_list]
        self.num_episode = len(self.episode_list)
        self.accumulated_num_step = list(accumulate(self.num_step_per_episode))
        self.dataset_len = self.accumulated_num_step[-1] if self.accumulated_num_step else 0
        
        self.rgb_preprocessor = rgb_preprocessor
        
    
    def load_primary_rgb(self, episode_id, step_id):
        """加载主视角图像"""
        image_path = f'{self.dataset_path}/episodes/{episode_id}/steps/{step_id}/{self.primary_mode}.jpg'
        image = np.array(Image.open(image_path).convert("RGB"))
        image = image[::-1, ::-1, :]
        return image.astype(np.uint8)
    
    def _get_episode_step(self, idx):
        """根据全局索引获取episode_id和step_id"""
        episode_idx = bisect.bisect_right(self.accumulated_num_step, idx)
        if episode_idx - 1 >= 0:
            step_idx = idx - self.accumulated_num_step[episode_idx - 1]
        else:
            step_idx = idx
        episode_id = self.episode_list[episode_idx]
        return episode_id, step_idx
    
    def extract_frames(self, idx, delta_t, rgb_initial, rgb_future, latent_mask):
        """提取序列帧"""
        episode_id, start_step = self._get_episode_step(idx)
        
        # 加载初始帧
        rgb_initial[0] = torch.from_numpy(
            rearrange(self.load_primary_rgb(episode_id, str(start_step).zfill(4)), 'h w c -> c h w')
        )
        if self.rgb_preprocessor is not None:
            rgb_initial[0] = self.rgb_preprocessor(rgb_initial[0])
        
        
        # 加载未来帧
        for i in range(self.sequence_length):
            next_step = start_step + (i+1) * delta_t
            try:
                # 检查是否超出该episode的步数
                episode_idx = bisect.bisect_right(self.accumulated_num_step, idx)
                max_steps = self.num_step_per_episode[episode_idx] + self.sequence_length * self.skip_frame
                if next_step >= max_steps:
                    break
                    
                next_step_str = str(next_step).zfill(4)
                
                # 加载RGB
                rgb_future[i] = torch.from_numpy(
                    rearrange(self.load_primary_rgb(episode_id, next_step_str), 'h w c -> c h w')
                )
                if self.rgb_preprocessor is not None:
                    rgb_future[i] = self.rgb_preprocessor(rgb_future[i])
                
                latent_mask[i] = 1
            except Exception as e:
                print(f"Error loading frame at episode {episode_id}, step {next_step}: {e}")
                break
    ...
    def obtain_item(self, idx, delta_t=None):
        ...
        if latent_mask.sum() == 0:
            raise Exception("latent_mask应该大于零!")
